...
from app.nodes.common import fill_lines
...
def codegen_multi_input(ctx: CodegenContext) -> str:
    ...
    for i, row in enumerate(rows):
        ...
        locator_expr = f"{ctx.target_var}.locator({selector_expr})"
        if kind == "select":
            lines.append(f"{locator_expr}.select_option({value_expr})")
            continue
        lines += fill_lines(locator_expr, value_expr, bool(row.get("clearFirst")), bool(row.get("simulateTyping")))
